...
class URL: #url example: http://example.org:80/index.html
    ...
    def request(self): #use fields genereated in constructor to generate an HTTP request to the server running under the domain..
        ...
        if self.protocol == "https":
            ...
            sock= ssl_context.wrap_socket(sock) #wrap in the ssl cntx to provide a secure HTTP connection
...
def show(resp_body): #display only the text within the HTML body returned by the request() method
        in_tags= False
        output=""            
        for c in resp_body:
